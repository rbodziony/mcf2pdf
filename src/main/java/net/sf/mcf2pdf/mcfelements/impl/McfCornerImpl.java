...
public class McfCornerImpl implements McfCorner {
	...
	public void setWhere(String text) {
		 ...
		 * 
		 * position = where.bottom_left; } else if ("bottom-right".equals(text)) {
		 * position = where.bottom_right; } else if ("top-left".equals(text)) { position
		 * = where.top_left; } else if ("top-right".equals(text)) { position =
		 * where.top_right; } else { System.out.println("unknown where: " + text); }
		 */
	}
